...
// 선택할 수 있는 색상 테마 목록
settings.colorTheme = localStorage.getItem("colorTheme") || "#3498db";
watchEffect(() => {
  document.documentElement.style.setProperty("--primary-color", settings.colorTheme);
...
// 설정 저장시 호출되는 함수
const saveSettings = () => {
  // 설정을 저장하는 로직을 여기에 추가 (예: API 전송 또는 localStorage 저장)
  showToast.value = true;

  setTimeout(() => {
    showToast.value = false;
  }, 3000); // 3초 뒤 토스트 메시지 사라짐
};
</script>
<style scoped>
html {
